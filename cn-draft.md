...
操作系统的调试，尤其是针对使用Rust编写的系统，面临着一系列独特的挑战。其中最显著的挑战之一便是调试跨特权级的代码。在操作系统代码执行过程中，特别是在用户态和内核态间的系统调用交互时，能够跨特权级跟踪代码的执行是至关重要的。然而，如果用户想利用 GDB 同时跟踪内核态和用户态代码，在 GDB 中同时设置内核态和用户态代码的断点，会发现由于特权级切换时页表进行了刷新，地址空间改变，而 GDB 断点又是依赖于地址空间的，所以内核态，用户态 GDB 断点无法全部生效。
...
如前文所述，我们通过断点组和符号表的动态切换来保证内核态，用户态断点均能正确触发。考虑到最终目标是实现在如VSCode这样的集成开发环境中，通过图形用户界面进行操作系统的调试，我们选择开发一个VSCode插件来控制GDB，以此实现“断点组切换”机制。在VSCode环境中，调试过程并非通过VSCode和调试器的直接通讯进行，而是依赖于称为调试适配器（Debug Adapter, DA）的中间件。调试适配器将不同调试器（例如GDB和LLDB）的接口协议统一抽象为调试适配器协议（Debug Adapter Protocol, DAP），而VSCode已对DAP提供了完善的支持。此外，基于DAP，VSCode实现了一个与编程语言无关的原生调试器用户界面。因此，开发者仅需实现一个与其调试器交互的调试适配器，并将之集成至VSCode即可。
...
正如前文所述，实现“断点组切换”机制依赖于两个“边界断点”。然而，一些Rust OS 项目如 rCore-Tutorial-v3 和 ArceOS 在编译操作系统镜像文件时采取了激进的优化策略，如函数内联和删除大量调试相关的符号信息，这使得边界断点难以被有效设置。因此，不对编译参数进行调整的情况下，这些 OS 编译生成的操作系统映像及其调试信息文件难以适用于基于“断点组切换”机制的调试需求。为了应对此问题，我们对编译参数进行了必要的修改，以最大限度减少编译器的优化行为。

大部分的 Rust OS 都支持release和debug两种编译模式。release模式倾向于应用较高级别的代码优化并且剔除较多的调试信息，而debug模式则采用较低级别的优化并保留更多调试信息，后者更符合我们的需求。然而，我们在适配一些Rust OS时发现，一些Rust OS，如rCore-Tutorial-v3，不支持以debug模式编译。鉴于此，我们修改了release模式的配置文件，使得编译器在release模式下也会保留调试信息且减少代码优化。

此外，为了进一步提高性能，一些 Rust OS 会修改链接脚本，导致包含调试信息的DWARF段，例如.debug_info，在链接过程中被忽略。因此我们也对链接脚本进行了调整，取消了对这些调试信息段的忽略。为了使这些修改生效，必须使用cargo clean命令来清除之前的编译缓存。

以上修改导致编译出的内核和用户态程序占用的磁盘空间显著增加。在某些以教学用途为主的操作系统中，可能会出现用户态程序文件过大，超出文件系统加载能力的情况。为解决此问题，我们调整了文件系统的磁盘打包程序的相关配置，如磁盘大小。同时，由于可执行文件中保留了大量符号信息，运行时占用的内存也相应增加，我们进一步调整了操作系统的用户堆栈大小和内核堆栈大小。

我们将对配置文件、链接脚本和操作系统源代码的所有修改汇总为一个diff文件，用户可通过git工具应用该diff文件以实现上述修改。同时，我们还维护了一些已经包含这些修改的 Rust OS 项目的GitHub仓库，方便用户直接下载和使用。

## 3 解决 Rust 操作系统中基于 eBPF 的内核态，用户态代码的动态跟踪调试

### 3.1 概述

在成功实现基于GDB的内核态与用户态代码的便捷切换跟踪之后，我们发现了该调试工具存在的限制。特别是，为了获得充分的调试信息，不得不修改编译参数，以取消函数内联并保留符号表（参见2.4节）。此做法导致内核与应用程序在内存和磁盘空间上的占用量大幅增加。虽然在虚拟机环境下，通过增加虚拟机分配的资源可以解决这一问题，但在内存资源受限的真实硬件上调试时，由于无法简单地增加硬件内存，因此操作系统可能因为过多的调试信息占用内存而无法正常运行，这对于用户而言是不可接受的。

鉴于此，我们探索了一种新的、更为高效灵活的调试信息获取方法。基于此目标，我们引入了基于eBPF技术的动态跟踪调试功能。实现这个功能的关键在于开发一套尽可能不影响操作系统运行的调试命令和调试信息传输通道，同时确保eBPF的调试功能能够由GDB控制，从而使得用户只通过GDB就可以同时使用 GDB 断点和 eBPF技术进行调试。

要想基于eBPF技术进行调试，被调试的操作系统必须要支持eBPF技术。由于我们想调试的rCore-Tutorial-v3操作系统的主线版本暂不支持eBPF，我们移植了现有的kprobe模块和eBPF虚拟机，并且将另一个 OS 的紧耦合的 uprobe 代码改写成一个独立的 Rust 模块（crate），并将其移植至 rCore-Tutorial-v3 ，同时提供了详细的移植指南[8]，便于他人将我们的 uprobe 模块移植到更多操作系统中。

通过引入基于eBPF的内核态与用户态代码的动态跟踪调试功能，用户将拥有更灵活的动态调试策略，能够获取更为丰富的调试信息，同时将调试过程对操作系统运行状态的影响降至最低（这有益于获取和运行时间相关的信息）。这对于在硬件资源受限且无法修改编译参数的环境中，仍然能够获取充分调试信息的需求至关重要。

接下来的章节将首先介绍rCore-Tutorial-v3上eBPF的移植过程，随后讨论如何将eBPF调试功能集成至VSCode编辑器。
...
在移植eBPF虚拟机、kprobe 的过程中，除了处理代码依赖的问题外，我们主要的工作是实现并优化符号解析功能，分为内核实现和外部插件实现。

##### 3.2.2.1 符号解析的内核实现

考虑到用户在大部分情况下都是使用函数名，变量名等符号而不是具体的内存地址来设置断点，内核中的eBPF模块需要有将符号转换为地址的功能。rCore-Tutorial-2022A的eBPF模块预留了这个功能的接口但并没有实现，而我们在rCore-Tutorial-v3的eBPF模块中实现了这个功能。
...
在有了完整且可解析的符号表并成功注入内核后，我们编写了一个内核函数用于在符号表中搜索特定的符号并返回符号对应的地址，从而实现了预留的符号解析接口。
