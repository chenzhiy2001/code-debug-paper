...
统一的调试会话中，针对内核态和任意多个用户态程序进行源码级断点调试。

本工作基于现有的 VSCode 用户态程序调试插件（Native Debug[参考文献]）进行扩展，增加了面向跨特权级以及多进程场景的断点组管理与切换机制。本章首先从整体出发，说明如何在原有插件中加入断点组管理模块以及上下文切换检测模块；随后重点描述断点组管理的数据结构、核心状态机的实现方法，以及自动单步、出口断点、钩子断点等关键功能。最后给出一个在 RISC-V 上对内核和任意多个用户态程序进行源码级断点调试的示例，展示如何在真实或模拟环境中使用本工具。

### 5.1 Overall Architecture 概述

我们所基于的 Native Debug 插件原本聚焦于单一用户态程序的调试流程。它在 VSCode 与 GDB 之间充当中间桥梁：VSCode 将用户的断点、单步、继续等指令转交给插件，插件再以调试命令的形式下发给 GDB，GDB 处理完毕后返回状态信息。这样的模式在仅有一个进程或不涉及特权级切换的场景下运行良好，但无法满足跨内核和多个用户进程的调试需求。

为实现跨隔离环境的断点与符号表切换，我们扩展了插件与 GDB 之间的数据通信流程，并在插件内部加入一个断点组管理模块（BreakpointGroups）与一个跨特权级的自动调度机制。断点组管理模块负责将用户所设置的断点信息进行分组存储与激活，而自动调度机制则需要在运行期间检测何时离开内核或当前用户进程，并在适当时机触发断点组切换。为了在实现层面更好地维护多态且频繁的断点状态，我们在插件内加入一个用于表征断点组切换过程的有限状态机 (OSStateMachine)。在此基础上，插件能够在“内核断点组”与“用户态进程断点组”之间无缝切换，并支持对多个用户进程的断点进行暂存或激活。

在底层执行环境方面，我们兼容真实硬件和 QEMU 模拟器两种形态。若在真实硬件上调试，插件会通过 OpenOCD 提供的远程调试接口将命令下发至目标 CPU；若在 QEMU 中，则借助 QEMU 内置的 GDBStub。在二者之上，我们对操作系统的类型（rCore-Tutorial、xv6、Starry 等）及其运行时切换特权级的具体方式不做过多假设，核心逻辑均由断点组管理和状态机来保障。

### 5.2 Core Modules

#### Breakpoint Groups

#### 5.2.1 BreakpointGroups：跨隔离环境的断点管理

#### 5.2.2 OSStateMachine：特权级切换检测与自动单步
 

### 5.3 Example & Experimental Setup 实验与示例

为展示本工具在多种场景下的功能性与适配性，我们做了如下实验性使用，验证其在不同 OS、不同硬件/虚拟机环境、以及不同编程语言下的可行性。表 1 列出了目前已测试或部分验证通过的配置。

| **操作系统**      | **硬件/虚拟机**            | **主要语言** | **CPU 架构** | **适配/移植需求**                 |
|-------------------|----------------------------|-------------|-------------|-----------------------------------|
| rCore-Tutorial    | QEMU (RISC-V) / OpenOCD    | Rust        | RISC-V      | 无需改动内核，仅在本插件中配置即可 |
| xv6               | QEMU / 真机 (RISC-V)       | C           | RISC-V      | 在内核 trap 处设置出口断点        |
| Starry            | QEMU (RISC-V)             | Rust + C    | RISC-V      | 在调度/进程创建位置添加钩子断点   |
| 其他 OS (同理)    | QEMU / 真实硬件 (可扩展)    | C/C++/Rust  | x86 / ARM 等 | 需在配置文件中给出断点组及符号映射 |

表 1. 本工具已验证或可行的运行环境示例  

#### 5.3.1 功能示例

1. **跨特权级单步调试**  
   - 在 rCore-Tutorial 中，当 OS 从内核返回用户态前，触发出口断点；状态机执行卸载内核断点 -> 单步 -> 检测到 PC 跳转到应用地址空间后，加载用户断点 -> 用户态代码触发断点成功。  
2. **捕获下一进程名**  
   - 在 Starry OS 中，调度函数设有一个“钩子断点”，被触发时自动读取某个内核变量（例如 “next_pid”），将其映射成断点组名，然后更新 `nextBreakpointGroup`。从而在多进程间来回切换时无需手动介入。  

<div align="center">  
图 2. VSCode 中调试 rCore-Tutorial 的示例截屏：图中展示用户态断点与内核断点先后被触发  
</div>  

#### 5.3.2 适配新环境的方法

若需要将本工具移植到一个新的 OS 或新的硬件平台（如 ARMv8），只需在配置文件中加入以下信息：  

- **断点组名到符号表文件路径的映射规则**；  
- **源代码文件到断点组名的映射规则**；  
- **出口断点 / 钩子断点的位置**；  
- （可选）若硬件环境不支持单步指令，需要以软件模拟单步；或若使用新的调试接口，需要重写与 GDB 相关的连接配置。

这使得跨不同 OS/硬件的调试工作主要以“配置导向（configuration-driven）”的方式进行，无需大改操作系统内部。  

---

综上，得益于断点组管理与状态机双管齐下，本工具成功地在多个 OS 环境下演示了“跨特权级、多进程”的断点调试能力，并在实验中展现了较好的易用性和可扩展性。下一节我们将总结本研究的局限与可能的未来扩展方向。  

---

> **附注**：上文提到的核心源代码文件可参见 `mibase.ts` 和 `OSStateMachine.ts`。其中 `BreakpointGroups` 类及其方法如 `saveBreakpointsToBreakpointGroup()`、`updateCurrentBreakpointGroup()` 等实现了断点缓存与符号表切换；`OSStateMachine` 则借助有限状态机的方式封装了跨特权级切换的自动单步与断点组更新逻辑。为了提高可读性，文中并未逐行罗列所有实现细节，感兴趣的读者可在附录查看更为完整的源码分析。 
