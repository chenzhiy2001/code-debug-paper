...
- 断点组管理模块是 GDB 上层的工具，会控制 GDB
- 断点组数据结构是什么样的
- 怎么判定一个断点属于哪个断点组：由于用户设置断点时，调试器能得到的信息只有断点所在文件名和行号，所以我们要求用户在配置文件中提供一个规则，设置文件名和文件名中代码所在的页表的映射关系。比如，所有在“initproc”文件夹下的代码文件都属于initproc进程的页表，用户可以在配置文件中将“initproc”文件夹下的代码文件都映射到一个叫做“initproc”的断点。
